...
// Target: Nexys 4 (Digilent, Xilinx Artix-7 XC7A100T-CSG324-3)
//
...
//     LED[2]  -> contaminated
//     LED[3]  -> result_valid
//     LED[6:4] -> family_id (binary: 001-101) //3-bit
//     LED[7]  -> loader active
//     LED[11:8] -> pesticide_id (1-8) //4-bit
//     LED[14:12] -> sample_select echo //3-bit
...
module top_nexys4 (
    ...
    // Mode-change detection: reset SVM when user switches modes
    reg [1:0]   prev_mode;
    reg         mode_changed;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            prev_mode    <= SW[15:14];
            mode_changed <= (SW[15:14] != prev_mode);
        end
    end
    
    // SVM effective reset: global AND bram-loader reset AND mode-change reset
    wire        svm_effective_rst_n = rst_n 
                                    & (bram_mode ? bram_svm_rst_n : 1'b1) 
                                    & ~mode_changed;
    ...
    //------------------------------------------------------------------
    // Power-On Reset + Manual Reset (BTNU)
    // Holds system in reset for ~10ms after FPGA configuration.
    // After power-on reset completes, BTNU can still trigger manual reset.
    //------------------------------------------------------------------
    reg [19:0] por_counter;
    reg        por_done;
    
    always @(posedge clk) begin
        if (!por_done) begin
            por_counter <= por_counter + 1;
            if (por_counter[19])    // ~5.2 ms at 100 MHz
                por_done <= 1'b1;
        end
    end
    
    // Reset active when: power-on reset NOT done, OR BTNU pressed
    assign rst_n = por_done & ~BTNU;
