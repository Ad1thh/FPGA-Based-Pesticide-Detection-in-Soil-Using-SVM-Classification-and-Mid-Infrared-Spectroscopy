...
module seven_seg_controller (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            // Show "  rEAdY " on reset — system ready for classification
            msg[0] <= CH_BLANK; msg[1] <= CH_BLANK;
            msg[2] <= CH_r;     msg[3] <= CH_E;
            msg[4] <= CH_A;     msg[5] <= CH_d;
            msg[6] <= CH_Y;     msg[7] <= CH_BLANK;
    ...
    //------------------------------------------------------------------
    // Anode driver (ACTIVE-LOW: drive LOW to select digit on Nexys 4)
    //------------------------------------------------------------------
    always @(*) begin
        an = 8'b1111_1111;          // All digits OFF (active-low)
        an[digit_select] = 1'b0;    // Selected digit ON (drive LOW)
    end
    
    //------------------------------------------------------------------
    // Cathode driver (ACTIVE-LOW: matches internal encoding directly)
    // Nexys 4 DDR cathodes are active-low, same as our encoding
    //------------------------------------------------------------------
    always @(*) begin
        seg = msg[7 - digit_select];  // Direct pass-through (both active-low)
    end
